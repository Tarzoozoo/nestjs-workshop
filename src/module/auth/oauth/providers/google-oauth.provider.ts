...
// - Implements methods to get authorization URL and handle callback
// - Fetches user info from Google APIs
@Injectable()
export class GoogleOAuthProvider implements OAuthProviderInterface {
  ...
  constructor(private configService: ConfigService) {
    this.config = {
      ...
      scope: this.configService.getOrThrow<string>('GOOGLE_SCOPE').split(' '),
      authURL: this.configService.getOrThrow<string>('GOOGLE_AUTH_URL'),
      tokenURL: this.configService.getOrThrow<string>('GOOGLE_TOKEN_URL'),
    };
  }
  ...
  private async exchangeCodeForToken(
    ...
    if (!response.ok) {
      const text = await response.text(); // log body จาก Google
  ...
  private async getUserInfo(accessToken: string) {
    ...
    if (!userInfoURL) {
      throw new Error('GOOGLE_USER_INFO_URL is not defined');
    }
